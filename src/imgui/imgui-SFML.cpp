...
void Update(float cameraScale, sf::RenderWindow& window, sf::Time dt) {
    Update(cameraScale, window, window, dt);
}

void Update(float cameraScale, sf::Window& window, sf::RenderTarget& target, sf::Time dt) {
    ...
    if (!s_mouseMoved) {
        ...
        Update(cameraScale, s_touchPos, static_cast<sf::Vector2f>(target.getSize()), dt);
    } else {
        Update(cameraScale, sf::Mouse::getPosition(window),
...
void Update(float cameraScale, sf::Vector2i mousePos, const sf::Vector2f& displaySize,
            sf::Time dt) {
    ImGuiIO& io = ImGui::GetIO();
    io.DisplaySize = displaySize;
    io.DeltaTime = dt.asSeconds();

    mousePos = sf::Vector2i(mousePos.x / cameraScale, mousePos.y / cameraScale);
