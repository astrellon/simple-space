...
namespace space
{
    MapLayer::MapLayer(const tmx::Map& map, ResourceManager &resourceManager, std::size_t idx)
    {
            ...
            idx < layers.size() && layers[idx]->getType() == tmx::Layer::Type::Tile)
        {
            ...
            createChunks(map, resourceManager, layer);
    ...
    MapLayer::ChunkPtr& MapLayer::getChunkAndTransform(int x, int y, sf::Vector2u& chunkRelative)
    {
    ...
    void MapLayer::createChunks(const tmx::Map& map, ResourceManager &resourceManager, const tmx::TileLayer& layer)
    {
        ...
        for (auto y = 0u; y < m_chunkCount.y; ++y)
        {
            ...
            for (auto x = 0u; x < m_chunkCount.x; ++x)
            {
                ...
                m_chunks.emplace_back(std::make_unique<Chunk>(layer, usedTileSets,
                    sf::Vector2f(x * m_chunkSize.x, y * m_chunkSize.y), tileCount, tileSize, map.getTileCount().x, resourceManager, map.getAnimatedTiles()));
            }
        }
    }
    ...
    // Chunk -------------------------------------------------------------------------
    MapLayer::Chunk::Chunk(const tmx::TileLayer& layer, const std::vector<const tmx::Tileset*> &tilesets,
        const sf::Vector2f& position, const sf::Vector2f& tileCount, const sf::Vector2u& tileSize,
        std::size_t rowSize,  ResourceManager& resourceManager, const std::map<std::uint32_t, tmx::Tileset::Tile>& animTiles)
            : m_animTiles(animTiles)
        {
            ...
            for (const auto& ts : tilesets)
            {
                ...
                sf::Texture *texture;
                if (!resourceManager.texture(ts->getImagePath(), &texture))
                {
                    std::cout << "Unable to find texture for chunk: " << ts->getImagePath() << std::endl;
                }
                m_chunkArrays.emplace_back(std::make_unique<ChunkArray>(*texture, *ts));
            }
